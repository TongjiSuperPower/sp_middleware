...
struct DartClientCmd
{
  ...
  uint16_t latest_launch_cmd_time;      // 发送最新飞镖发射指令的比赛剩余时间，单位：秒，无/未发射，默认为 0。
};
...
struct SentryInfo
{
  ...
  uint32_t immediate_resurrect_cost    : 10;      // bits 21-30 : 哨兵机器人当前若兑换立即复活需要花费的金币数
  uint32_t reserved                    : 1;       // bit 31     : 保留
  uint16_t is_out_of_battle            : 1;       // bit 0      : 哨兵当前是否处于脱战状态
  uint16_t team_17mm_remaining_exchanges : 11;    // bits 1-11  : 队伍 17mm 允许发弹量的剩余可兑换数
  uint16_t reserved                    : 4;       // bits 12-15 : 保留
};

// 0x020E 雷达自主决策信息同步
struct RadarInfo
{
  uint8_t double_vul_trigger_chance     : 2; // bits 0-1 : 雷达拥有触发双倍易伤的机会（0~2），开局为 0，最大可达 2
  uint8_t opponent_in_double_vulnerability : 1; // bit 2 : 对方是否正在被触发双倍易伤 0：对方未被触发双倍易伤 1：对方正在被触发双倍易伤
