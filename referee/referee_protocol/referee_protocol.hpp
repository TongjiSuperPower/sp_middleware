...
constexpr uint16_t ROBOT_CLIENT_DATA =
  0x0310;  // 机器人发送给自定义客户端的数据 图传链路 (最大300字节)
...
struct __attribute__((packed)) EventData
{
  ...
  // bit 25-26：己方堡垒增益点的占领状态，0 为未被占领，1 为被己方占领，2 为被对方占领，3 为被双方占领。（仅 RMUL 适用）
  uint32_t outpost_boost_status : 2;  // bit 27-28
  // bit 27-28：己方前哨站增益点的占领状态，0 为未被占领，1 为被己方占领，2 为被对方占领
  uint32_t base_boost_status : 1;  // bit 29
...
{
  uint8_t dart_remaining_time;  //己方飞镖发射站剩余飞镖发射时间，单位：s
  uint16_t dart_info;
...
struct __attribute__((packed)) RFID_Status
{
  ...
  uint8_t enemy_tunnel_road_down : 1;       // bit 0
  uint8_t enemy_tunnel_road_mid : 1;        // bit 1
  uint8_t enemy_tunnel_road_up : 1;         // bit 2
  uint8_t enemy_tunnel_trapezoid_low : 1;   // bit 3
  uint8_t enemy_tunnel_trapezoid_mid : 1;   // bit 4
  uint8_t enemy_tunnel_trapezoid_high : 1;  // bit 5
...
struct __attribute__((packed)) RadarMarkData
{
  ...
  uint16_t enemy_uav_vulnerable : 1;         // bit 4: 对方空中机器人易伤情况
  uint16_t enemy_sentry_vulnerable : 1;      // bit 5: 对方哨兵机器人易伤情况
  uint16_t friendly_hero_1_special : 1;      // bit 6: 己方 1号英雄机器人特殊标识情况
  uint16_t friendly_engineer_2_special : 1;  // bit 7: 己方 2号工程机器人特殊标识情况
  uint16_t friendly_infantry_3_special : 1;  // bit 8: 己方 3号步兵机器人特殊标识情况
  uint16_t friendly_infantry_4_special : 1;  // bit 9: 己方 4号步兵机器人特殊标识情况
...
struct __attribute__((packed)) SentryInfo
{
  ...
  uint16_t reserved : 1;                  // bit 15    : 保留位
};
