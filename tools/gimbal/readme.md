...
# 目前思路
1.操作手仍然使用双环pid   即target_angle ---target_speed---target_torque
2.自瞄要对视觉传来的数据(角加速度)进行参考系变换并且考虑底盘的牵连角加速度(通过底盘角速度带入pid计算出一个角加速度) 
再并联上一个小pid 对稳态误差进行补偿 (可能包含了yaw的重力矩)
最后得到的这个总角加速度 带入MPC力矩控制算出力矩
# 使用方法
