...
constexpr float MF9025_CURRENT_TO_TORQUE = 0.32f;

LK_Motor::LK_Motor(uint8_t motor_id) : rx_id(0x140 + motor_id), tx_id(0x280), motor_id_(motor_id) {}
...
void LK_Motor::read(const uint8_t * data)
{
  ...
  this->temp = data[1];
  this->torque = torque_int * MF_RAW_TO_CURRENT * MF9025_CURRENT_TO_TORQUE;
...
{
  float raw = value / MF9025_CURRENT_TO_TORQUE / MF_RAW_TO_CURRENT;
